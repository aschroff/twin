...
| **Raycast must hit mesh** | Painting only occurs if the screen-space ray hits a collider on `hitScreen.Layers`. |
| **The twin's saved camera decides whether the centre hits the body** | `DragOnCanvas` paints at the screen centre, but a twin restores its own camera on load. The LipEdema sample restores its "Lower body" view (ortho size 0.67 at y≈0.43) where the centre falls **between the legs** — the ray misses and no part is created, silently. Select a framing view first (`TwinPaintTestBase.SelectView(BodyView)`), or compute the position from a bone as the template generator does. |
...
| **Replaying serialized commands** | `CommandData.PaintableTexture` is serialized as an instanceID → always null after a scene reload. Since 2026-07-31 `PartManager.LoadData` re-binds null references automatically (regression test: `PartTemplateServiceTests.LoadTwin_BindsCommandsToPaintableTexture`); only code that bypasses `LoadData` needs to re-bind manually. Also: the FIRST replay in a harness session can render misaligned ("crescents") — do one live warm-up paint first, then Erase+Refresh. Harness-only (in-app replay renders correctly); suspect lazy skinned-mesh bake. The serialized data itself round-trips byte-perfectly. A replay issued right after a twin was loaded can render **nothing at all**, even with a warm-up paint earlier in the test: measured on the same twin, a `Refresh()` one frame after the load changed 0 texture pixels while the identical call ~1 s later changed all 14058 of them. Give the body time to settle before replaying and before reading the texture — `PartsScreenshotProcess` waits 0.5 s on each side of its `ClearRefreshPart`. |
