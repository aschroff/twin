...
| **Replaying serialized commands** | `CommandData.PaintableTexture` is serialized as an instanceID → always null after a scene reload; re-bind to the live `CwPaintableMeshTexture` before `PartManager.Refresh()`. Also: the FIRST replay in a session can render misaligned ("crescents") — do one live warm-up paint first, then Erase+Refresh. Root cause open (suspect lazy skinned-mesh bake); the serialized data itself round-trips byte-perfectly. |
