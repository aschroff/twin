...
namespace NoAPICalls
{
    ...
    public class TextureSaveCostTests : TwinPaintTestBase
    {
        ...
        public IEnumerator SavingThePaintedTexture_CostsThisMuchPerStep()
        {
            ...
            long start = MemoryMeasure.Total();
            var report = new System.Text.StringBuilder();
            report.AppendLine("Cost of saving one " + paintable.Width + "x" + paintable.Height + " paint texture");
            report.AppendLine("  start                        = " + MemoryMeasure.Mb(start));

            // 1. the readable copy: a render texture plus a Texture2D, both full size
            Texture2D copy = paintable.GetReadableCopy();
            long afterCopy = MemoryMeasure.Total();
            report.AppendLine("  GetReadableCopy   " + MemoryMeasure.Signed(afterCopy - start).PadLeft(9)
                              + "  = " + MemoryMeasure.Mb(afterCopy));

            // 2. the PNG
            byte[] png = copy.EncodeToPNG();
            long afterPng = MemoryMeasure.Total();
            report.AppendLine("  EncodeToPNG       " + MemoryMeasure.Signed(afterPng - afterCopy).PadLeft(9)
                              + "  = " + MemoryMeasure.Mb(afterPng)
                              + "   (PNG is " + MemoryMeasure.Mb(png.LongLength) + ")");

            // 3. base64 - a C# string is two bytes per character, so this is 2.67x the PNG
            string base64 = System.Convert.ToBase64String(png);
            long afterBase64 = MemoryMeasure.Total();
            report.AppendLine("  ToBase64String    " + MemoryMeasure.Signed(afterBase64 - afterPng).PadLeft(9)
                              + "  = " + MemoryMeasure.Mb(afterBase64)
                              + "   (string is " + MemoryMeasure.Mb(base64.Length * 2L) + ")");

            // 4. PlayerPrefs, which keeps its own copy for as long as the app runs
            PlayerPrefs.SetString("TextureSaveCostScratch", base64);
            long afterPrefs = MemoryMeasure.Total();
            report.AppendLine("  PlayerPrefs.Set   " + MemoryMeasure.Signed(afterPrefs - afterBase64).PadLeft(9)
                              + "  = " + MemoryMeasure.Mb(afterPrefs));

            report.AppendLine("  TOTAL             " + MemoryMeasure.Signed(afterPrefs - start).PadLeft(9));

            PlayerPrefs.DeleteKey("TextureSaveCostScratch");
            Object.Destroy(copy);
        ...
        public IEnumerator SavingWithoutTheWastedCopies_CostsLess()
        {
            ...
            long afterCopy = MemoryMeasure.Total();
            report.AppendLine("  readable copy     " + MemoryMeasure.Signed(afterCopy - start).PadLeft(9)
                              + "  = " + MemoryMeasure.Mb(afterCopy));
            ...
            report.AppendLine("  EncodeToPNG       " + MemoryMeasure.Signed(afterPng - afterCopy).PadLeft(9)
                              + "  = " + MemoryMeasure.Mb(afterPng)
                              + "   (PNG is " + MemoryMeasure.Mb(png.LongLength) + ")");
            report.AppendLine("  TOTAL             " + MemoryMeasure.Signed(afterPng - start).PadLeft(9));
