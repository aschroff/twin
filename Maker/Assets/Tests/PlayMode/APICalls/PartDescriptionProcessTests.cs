...
namespace APICalls {
public class PartDescriptionProcessTests : PlayModeTestBase
{
    private const string Variant = "Medical Report"; // matches the "Medical Report (Part Description)" ItemPrompt in Settings UI

    [UnityTest]
    public IEnumerator DescribePart_SetsDescriptionFromRealScreenshot()
    {
        ...
        // PartDescriptionProcess only calls the AI if a screenshot for the part already exists on disk,
        // so produce a real one first via PartsScreenshotProcess - the same order VersionSequenceProcess uses.
        ...
        var descriptionProcess = Object.FindFirstObjectByType<PartDescriptionProcess>();
        Assert.IsNotNull(descriptionProcess, "PartDescriptionProcess not found in scene.");

        descriptionProcess.Handle(Variant + "##" + part.id);

        yield return WaitUntilOrTimeout(
            () => !string.IsNullOrEmpty(part.description),
            30f,
            "PartDescriptionProcess did not set a description in time.");
