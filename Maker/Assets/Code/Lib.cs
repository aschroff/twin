...
public class Lib : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (Input.GetMouseButtonDown(0))
        {
            ...
            DrawFinding(p0n, p1n, p2n);

            Debug.DrawLine(p0n, p1n, Color.red, 100.0f);
            Debug.DrawLine(p1n, p2n, Color.red, 100.0f);
            Debug.DrawLine(p2n, p0n, Color.red, 100.0f);

            float p0x = mesh.uv[triangles[hit.triangleIndex * 3 + 0]].x;
            float p0y = mesh.uv[triangles[hit.triangleIndex * 3 + 0]].y;
            float p1x = mesh.uv[triangles[hit.triangleIndex * 3 + 1]].x;
            float p1y = mesh.uv[triangles[hit.triangleIndex * 3 + 1]].y;
            float p2x = mesh.uv[triangles[hit.triangleIndex * 3 + 2]].x;
            float p2y = mesh.uv[triangles[hit.triangleIndex * 3 + 2]].y;
