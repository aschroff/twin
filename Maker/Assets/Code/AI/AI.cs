...
public class AI : MonoBehaviour {
    ...
    private void Start() {
        ...
        ChatGPTwin.Request(prompt, parameters, completeCallback: text => {
            ...
            _parametersWithCharacterRole = new ChatGptParameters(parameters) { role = text };
    ...
    private void AddUserQuestion() {
        ...
        ChatGPTwin.Request(conversationAndRequest, parameters, updateCallback: chunkOfText => {
    ...
    private void AddNpcAnswer() {
        ...
        ChatGPTwin.Request(_conversationSoFar, _parametersWithCharacterRole, updateCallback: text => {
            // The request is not yet complete, so we're receiving chunks of text. Append them to the text field.
            textField.text += text;
            scrollRect.normalizedPosition = new Vector2(0, 0);
